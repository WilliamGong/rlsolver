...
log_param('n_steps', 2**args.n_steps)
log_param('batch_size', 2**args.batch_size)
log_param('n_epochs', args.n_epochs)
log_param('gamma', args.gamma)
log_param('gae_lambda', args.gae_lambda)
log_param('clip_range', args.clip_range)
log_param('ent_coef', args.ent_coef)
log_param('total_timesteps', args.timesteps)
...
for i in range(100):
    ...
    log_metric('objective', env.getObj())
    log_metric('reward', rewards)
