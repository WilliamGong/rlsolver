...
if __name__ == '__main__':
    ...
    # flags
    max_unchange_count = 20
    obj_unchanged_count = 0

    # openbayes tools
    log_param('learning_rate_A', LR_A)
    log_param('learning_rate_A', LR_C)
    log_param('epsilon', EPSILON)
    log_param('gamma', GAMMA)
    log_param('target_network_replace_freq', TARGET_NETWORK_REPLACE_FREQ)
    log_param('mamory_capacity', MEMORY_CAPACITY)
    log_param('max_episodes', MAX_EPISODES)
    log_param('max_ep_steps', MAX_EP_STEPS)
    log_param('exploration_var', VAR)
    ...
    for i in range(MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            ...
            if ddpg.pointer > MEMORY_CAPACITY:
                ...
                losses_index.append(losses_counter)
                losses_counter += 1
                a_losses.append(a_loss)
                td_losses.append(td_err)

                log_metric('a_loss', a_loss)
                log_metric('td_err', td_err)
            ...
            if j == MAX_EP_STEPS - 1:
                ...
                if len(objs) == 0:
                    ...
                    if objs[-1] == obj and VAR <= 1e-4:
                        obj_unchanged_count += 1
                    else:
                        obj_unchanged_count = 0
                ...
                log_metric('objective', obj)
                log_metric('reward', ep_reward)

                writer.add_scalar('objective', obj, i)
                writer.add_scalar('reward', ep_reward, i)
                break

        # exit when obj dose not change
        if obj_unchanged_count >= max_unchange_count:
            break
